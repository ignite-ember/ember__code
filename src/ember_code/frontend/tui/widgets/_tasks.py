...
class TaskPanel(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self._tasks: list = []  # task dicts/objects from BE
    ...
    def _render_all(self) -> None:
        ...
        try:
            ...
            if not self._tasks:
                ...
                active = sum(1 for t in self._tasks if t.status in ("pending", "running"))
                title.update(
    ...
    def _render_entry(self, index: int, task) -> str:
        status = getattr(task, "status", "") or ""
        # Handle enum objects (from direct Session) and strings (from RPC)
        if hasattr(status, "value"):
            status = status.value
        icon = _STATUS_ICONS.get(status, "?")
        arrow = "▼" if index in self._expanded else "▶"
        sched = getattr(task, "scheduled_at", "")
        time_str = (
            sched.strftime("%Y-%m-%d %H:%M") if hasattr(sched, "strftime") else str(sched)[:16]
        )
    ...
    def on_key(self, event) -> None:
        ...
        if event.key == "up":
            ...
            st = getattr(task.status, "value", task.status) if hasattr(task, "status") else ""
